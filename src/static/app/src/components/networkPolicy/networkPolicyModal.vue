	...
	methods: {
		...
		async loadPolicy(){
			if (!this.tunnelAddress){
				this.error = "This Peer needs a single-host Allowed IP before a forwarding policy can be managed.";
				this.loading = false;
				return;
			}
		...
		async preview(){
			this.error = "";
			if (!this.tunnelAddress){
				this.error = "Select a single-host tunnel address first.";
				return;
			}
		...
		async apply(){
			if (this.previewRequired){
				this.error = "Preview the generated rules before applying this policy.";
				return;
			}
			this.applying = true;
			this.error = "";
			await fetchPost("/api/networkPolicy/apply", this.basePayload(), (res) => {
				if (res.status){
					this.store.newMessage("WGDashboard", "Network policy applied", "success");
					this.previewRequired = true;
					this.previewRuleset = "";
					this.loadPolicy();
				}else{
		...
		async deactivate(){
			...
			await fetchPost("/api/networkPolicy/deactivate", this.basePayload(), (res) => {
				if (res.status){
					this.store.newMessage("WGDashboard", "Network policy disabled", "success");
		...
		async rollback(revisionId){
			...
			await fetchPost("/api/networkPolicy/rollback", {revision_id: revisionId}, (res) => {
				if (res.status){
					this.store.newMessage("WGDashboard", "Network policy rolled back", "success");
					this.loadPolicy();
				}else{
					this.error = res.message;
				}
	...
	<div class="peerSettingContainer w-100 h-100 position-absolute top-0 start-0 overflow-y-scroll">
		<div class="dashboardModal bg-body shadow mx-auto my-4 p-4">
			<div class="d-flex align-items-start gap-3 mb-4">
				...
				<button type="button" class="btn-close ms-auto" title="Close" @click="$emit('close')"></button>
			</div>
			...
			<div class="mb-3">
				...
				<div class="form-text">Only a single-host Allowed IP can be bound to a forwarding policy.</div>
			</div>
			...
			<div v-if="policy.managed" class="border-top pt-3">
				<div class="d-flex align-items-center mb-2">
					...
					<button type="button" class="btn btn-sm btn-outline-primary ms-auto" title="Add rule" @click="addRule"><i class="bi bi-plus-lg"></i></button>
				</div>
				<div v-for="(rule, index) in policy.rules" :key="index" class="rule-row border-bottom py-3">
					<div class="row g-2 align-items-end">
						<div class="col-12 col-md-5">
							<label class="form-label small">Destination IP or CIDR</label>
						...
						<div class="col-5 col-md-3">
							...
							<div v-if="rule.ports === null" class="form-control text-muted">All ports</div>
							<div v-else class="d-flex gap-1"><input class="form-control" type="number" min="1" max="65535" v-model.number="rule.ports.from" placeholder="From"><input class="form-control" type="number" min="1" max="65535" v-model.number="rule.ports.to" placeholder="To"></div>
						</div>
						<div class="col-1 col-md-2 d-flex justify-content-end gap-1">
							<button type="button" class="btn btn-sm btn-outline-secondary" :title="rule.ports === null ? 'Use port range' : 'Allow all ports'" @click="setAllPorts(rule, rule.ports !== null)"><i :class="rule.ports === null ? 'bi bi-list-ol' : 'bi bi-infinity'"></i></button>
							<button type="button" class="btn btn-sm btn-outline-danger" title="Remove rule" @click="removeRule(index)"><i class="bi bi-trash"></i></button>
						</div>
					</div>
				</div>
				<div v-if="policy.rules.length === 0" class="small text-warning py-2">No destination is allowed. Applying this policy denies all forwarded traffic for this Peer.</div>
			</div>
			...
			<div v-if="previewRuleset" class="mt-3">
				<div class="small fw-bold mb-1">Generated nftables rules <code>{{ previewHash }}</code></div>
			...
			<div v-if="revisions.length" class="mt-4 border-top pt-3">
				...
				<div v-for="revision in revisions" :key="revision.revision_id" class="d-flex gap-2 align-items-center py-2 border-bottom small">
					<span :class="revision.status === 'applied' ? 'text-success' : revision.status === 'failed' ? 'text-danger' : 'text-warning'">{{ revision.status }}</span>
					<span>v{{ revision.version }} · {{ revision.action }}</span>
					<code class="text-muted text-truncate">{{ revision.hash }}</code>
					<button type="button" class="btn btn-sm btn-outline-secondary ms-auto" title="Restore this revision" :disabled="applying" @click="rollback(revision.revision_id)"><i class="bi bi-arrow-counterclockwise"></i></button>
				</div>
			</div>
		</div>
	</div>
