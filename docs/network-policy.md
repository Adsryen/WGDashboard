...
2. Add explicit TCP, UDP, or ICMP destination rules. Every configured destination automatically permits ICMP diagnostics for that destination only. Use an ICMP rule when diagnostics should be allowed without opening a TCP or UDP service. An empty rule list denies all forwarded traffic for the Peer.
